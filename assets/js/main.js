/* ============================================================
   IKAMABA Hub — Main JavaScript (Global)
   ============================================================ */
...
const Icons = {
  ...
  heartFilled: '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>',
  ...
  shoppingBag: '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>',
  ...
  trendingUp: '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>',
...
function initTabs() {
  document.querySelectorAll('[data-tabs]').forEach(tabContainer => {
    ...
    const contents = document.querySelectorAll(`[data-tab-content="${contentId}"] .tab-content`);
...
window.addToCart = function(name, price) {
  // Generate a unique ID based on string hash or just use name as ID for simplicity
  const id = name.replace(/\s+/g, '-').toLowerCase();
  Cart.addItem({ id: id, name: name, price: price, qty: 1 });
};
...
function initCountdown(targetDate, containerId) {
  ...
  function update() {
    ...
    if (diff <= 0) {
      container.innerHTML = '<p style="font-size:1.25rem;font-weight:700;color:var(--maroon);">Event Telah Dimulai! 🎉</p>';
      return;
    }
    ...
    container.querySelector('[data-days]').textContent = String(days).padStart(2, '0');
    container.querySelector('[data-hours]').textContent = String(hours).padStart(2, '0');
    container.querySelector('[data-minutes]').textContent = String(minutes).padStart(2, '0');
    container.querySelector('[data-seconds]').textContent = String(seconds).padStart(2, '0');
  }
...
// ── Set Active Nav Link & ScrollSpy ──
function setActiveNavLink() {
  const currentPage = window.location.pathname.split('/').pop() || 'index.html';
  const navLinks = document.querySelectorAll('.nav-link');
  
  // Set active for exact page match
  ...
  // ScrollSpy logic for hash links (e.g. #tentang, #event)
  if (currentPage === 'index.html' || currentPage === '') {
    const spySections = ['beranda', 'tentang', 'event'];
    ...
    window.addEventListener('scroll', () => {
      ...
      spySections.forEach(id => {
        ...
        if (section) {
          // Add 50px buffer to trigger earlier when scrolling down
